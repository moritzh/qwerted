...
public class DictionaryAccessTest extends junit.framework.TestCase {
    ...
    public void writeChildrenWorks() throws Exception {
        ...
        f.write();
        final DictionaryItem g = DictionaryItem.create("bcdasdfasdfasdfüüü");
        g.write();
        final DictionaryItem j = DictionaryItem.create("baüüüaaas");
        j.write();
        final DictionaryItem y = DictionaryItem.lookup("baü", false);
        f = d.readFirst();
    ...
    public void stringOperationsComparison() {
        final String foo = "ŠŸaaoossdf";
        assertTrue("charTo and the first of toCharArray are identical",
                foo.charAt(0) == foo.toCharArray()[0]);
        assertTrue("First char is utf8 and working!", (foo.charAt(0) == 'Š')
                && (foo.toCharArray()[0] == 'Š')
                && (foo.substring(1).toCharArray()[0] == 'Ÿ'));
